...
const WritePost = () => {
  ...
  const registerProduct = async () => {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('등록되었습니다');
        navigation.navigate('HomeTab'); // Root 화면으로 이동
  ...
  const categories = ['주방용품', '가구/인테리어', '패션잡화', '미용소품', '유아용품', '생활용품', '생활가전', '도서/문구', '미술품', '구합니다', '디지털기기', '스포츠/레저', '운동기구', '파티용품', '반려동물용품', '기타']; // 카테고리 목록
